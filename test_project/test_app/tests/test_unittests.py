...
class TestPlusViewTests(TestCase):
    ...
    def test_make_user_with_perms(self):
        ...
        self.assertEqual(list(u2.user_permissions.values_list("codename", flat=True)), ["add_group"])
    ...
    def test_post_is_ajax(self):
        response = self.post("view-is-ajax", data={"item": 1}, extra={"HTTP_X_REQUESTED_WITH": "XMLHttpRequest"})
        self.response_200(response)
...
class TestAPITestCaseDRFInstalled(APITestCase):
    def test_post(self):
        ...
        response = self.client.post(self.reverse("view-json"), data=json.dumps(data), content_type="application/json")
