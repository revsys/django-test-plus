...
class TestPlusViewTests(TestCase):
    ...
    def test_make_user_with_perms(self):
        ...
        self.assertEqual(
            list(u2.user_permissions.values_list("codename", flat=True)), [u"add_group"]
        )
    ...
    def test_get_is_ajax(self):
        response = self.get(
            "view-is-ajax", extra={"HTTP_X_REQUESTED_WITH": "XMLHttpRequest"}
        )
...
class TestPlusCBDataViewTests(CBVTestCase):
    ...
    def test_get_request_attributes(self):
        ...
        self.get(
            CBDataView, request=request, pk=self.data.pk, data=data,
        )
    ...
    def test_get_override_view_template(self):
        ...
        self.get(
            CBDataView, request=request, pk=self.data.pk,
        )
        ...
        self.get(
            ...
            request=request,
            pk=self.data.pk,
            initkwargs={"template_name": "other.html",},
        )
...
class TestPlusCBCustomMethodTests(CBVTestCase):
    def test_custom_method_with_value(self):
        ...
        instance = self.get_instance(
            CBView, initkwargs={"special_value": special_value}
        )
